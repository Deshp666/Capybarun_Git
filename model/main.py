...
class SoundLogic:
    def __init__(self):
        self.__sound_turned_on: bool = False

    def get_sound_condition(self) -> bool:
        return self.__sound_turned_on

    def toggle_sound(self, sound: pg.mixer.Sound):
        if self.__sound_turned_on:
            self.__sound_turned_on = False
            sound.set_volume(0)
        else:
            self.__sound_turned_on = True
            sound.set_volume(BASIC_VOLUME)

    @staticmethod
    def play_jump_sound(is_capybara_on_ground: bool, pause_condition: bool, sound: pg.mixer.Sound):
        if is_capybara_on_ground and not pause_condition:
            sound.play()
...
class Model:
    def __init__(self,
        ...
        self.sound_logic = SoundLogic()
        self.runner_logic = RunnerLogic()
        self.maze_logic = MazeLogic(maze_width, maze_height, indentation_x, indentation_y)

    def update_death_count(self):
        self.__death_count += 1
        self.maze_logic.set_death_count(self.__death_count)
